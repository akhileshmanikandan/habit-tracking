...
import { SignOut, Copy, Users, Shield, Fire, TreeEvergreen, ArrowsClockwise } from "@phosphor-icons/react";
import { haptics } from "@/lib/utils/haptics";

export default function ProfilePage() {
  const { user, userId, signOut } = useAuth();
  const { activeGroup: group, members, clearGroup, groups } = useGroupContext();
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-white/60 rounded-2xl border border-white/40 p-5 flex items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-sage/20 flex items-center justify-center overflow-hidden">
          {user?.avatar_url ? (
            <img
              src={user.avatar_url}
              alt={user.username}
      ...
      <motion.button
        ...
        className="w-full py-3 rounded-xl bg-white/60 border border-white/40 text-moss font-medium flex items-center justify-center gap-2"
      >
        <ArrowsClockwise weight="bold" className="w-4 h-4" />
        {groups.length > 1 ? "Switch Group" : "Manage Groups"}
      </motion.button>
